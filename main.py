...
class node:
    ...
    def drawNode(self, color):
        n = pygame.draw.circle(DISPLAYSURF,color,self.pos,node_size)
        return n
...
def generateNodes():
    ...
    for i in range(0,dim[0]):
        for j in range(0,dim[1]):
            nodes.append(node( (dim[0]*i) + j, (i*h + node_size,j*v + node_size) ))
    return nodes
...
nodes = generateNodes()
generateEdges()
drawNodes(nodes,[255,0,0])
print(mAdy)
mAdy.drawEdges([255,0,0])
